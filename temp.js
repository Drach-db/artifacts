    (function(){
      ...
      function renderOptionGroup({ containerSelector, items, selectedSet, defaultIds = [] }){
      ...
      function initCalculator(){
        ...
        // Events placeholder (telemetry removed)
      ...
      function initOffer(){
        ...
        const ensureHiddenDataField = () => {
          if (!hiddenForm) return null;
          let field = hiddenForm.querySelector('#offerData') || hiddenForm.querySelector('input[name="Данные"]');
          if (!field) {
        ...
        const hiddenData = ensureHiddenDataField();

        const getTrimmed = (input) => (input && typeof input.value === 'string') ? input.value.trim() : '';
        ...
        if (btnSend && email) {
          btnSend.addEventListener('click', () => {
            const trimmedEmail = getTrimmed(email);
            const isValid = /.+@.+\..+/.test(trimmedEmail);
            if (!isValid) {
              showNotice('Please enter a valid email.');
              email.focus();
              return;
            }
            if (!hiddenForm || !hiddenEmail || !hiddenData) {
              showNotice('Form handler is not configured. Please try again later.');
              return;
            }
            hiddenEmail.value = trimmedEmail;
            if (hiddenName) hiddenName.value = getTrimmed(name);
            if (hiddenCompany) hiddenCompany.value = getTrimmed(company);
            hiddenData.value = buildOfferSummary();
            if (hiddenSubmit) hiddenSubmit.click(); else hiddenForm.submit();
            resetVisibleFields();
            if (typeof openOfferSuccessModal === 'function') {
      ...
      function bindSelectable(listSelector, selectedSet, opts){
        const { limit = null, lockedSet = new Set(), limitMsg, lockedMsg } = opts || {};
      ...
      function formatCurrency(amount, currency){
        const fallbackCurrency = String(currency || 'USD').toUpperCase();
        ...
        try {
          ...
          return `${fallbackCurrency} ${rounded}`;
        }
      }

      function getCalcSummaryData(){
        const defaults = calcDefaults || {};
        const snapshot = calcStateSnapshot || {};
        const team = Number(snapshot.team ?? defaults.team ?? defaults.team_size ?? 0);
        const hoursPerDay = Number(snapshot.hoursPerDay ?? defaults.hours_per_day ?? 0);
        const daysPerWeek = Number(snapshot.daysPerWeek ?? defaults.days_per_week ?? 0);
        const inhouseRate = Number(snapshot.inhouseRate ?? defaults.inhouse_rate ?? 0);
        const currency = String(snapshot.currency ?? defaults.currency ?? 'USD').toUpperCase();
        const rateFromState = Number(snapshot.hourlyRate);
        let hourlyRate = Number.isFinite(rateFromState) ? rateFromState : NaN;
        if (!Number.isFinite(hourlyRate)) {
          if (calcRates && Object.prototype.hasOwnProperty.call(calcRates, currency)) {
            hourlyRate = Number(calcRates[currency]);
          } else if (DATA?.calculator?.currencies?.[currency] != null) {
            hourlyRate = Number(DATA.calculator.currencies[currency]);
          } else if (calcRates && Object.prototype.hasOwnProperty.call(calcRates, 'USD')) {
            hourlyRate = Number(calcRates.USD);
          } else if (DATA?.calculator?.currencies?.USD != null) {
            hourlyRate = Number(DATA.calculator.currencies.USD);
          } else {
            hourlyRate = 0;
          }
        }
        const hoursWeek = hoursPerDay * daysPerWeek;
        const monthly = Math.max(0, team * hourlyRate * hoursWeek * 4.33);
        const annual = Math.max(0, monthly * 12);
        const hasInhouse = Number.isFinite(inhouseRate) && inhouseRate > 0;
        const inhouseMonthly = hasInhouse ? Math.max(0, team * inhouseRate * hoursWeek * 4.33) : null;
        const savingsAbs = hasInhouse ? Math.max(0, (inhouseMonthly ?? 0) - monthly) : null;
        const savingsPct = hasInhouse && (inhouseMonthly ?? 0) > 0
          ? Math.max(0, Math.round((savingsAbs / inhouseMonthly) * 100))
          : null;
        return {
          team,
          hoursPerDay,
      ...
      function buildOfferSummary(){
        const calc = getCalcSummaryData();
        const roleName = selectedRole ? roleLabel(selectedRole) : '—';
        const kpiLabels = Array.from(kpiSelected).map(kpiLabel).filter(Boolean);
        const problemLabels = Array.from(problemSelected).map(problemLabel).filter(Boolean);

        const summaryParts = [
          `Role: ${roleName || '—'}`,
          `KPIs: ${kpiLabels.length ? kpiLabels.join(', ') : '—'}`,
          `Problems: ${problemLabels.length ? problemLabels.join(', ') : '—'}`,
          `Team size: ${formatInteger(calc.team)}`,
          `Coverage: ${formatInteger(calc.hoursPerDay)}h/day × ${formatInteger(calc.daysPerWeek)}d/wk`,
          `Currency: ${calc.currency}`,
          `Hourly rate: ${formatInteger(calc.hourlyRate)}`
        ];

        summaryParts.push(`Monthly (offer): ${formatCurrency(calc.monthly, calc.currency)}`);
        summaryParts.push(`Annual (offer): ${formatCurrency(calc.annual, calc.currency)}`);

        if (calc.hasInhouse) {
          summaryParts.push(`In-house $/hr: ${formatInteger(calc.inhouseRate)}`);
          summaryParts.push(`In-house monthly: ${formatCurrency(calc.inhouseMonthly, calc.currency)}`);
          const savingsAmount = formatCurrency(calc.savingsAbs ?? 0, calc.currency);
          const savingsPct = formatInteger(calc.savingsPct ?? 0);
          summaryParts.push(`Savings: ${savingsAmount} (${savingsPct}%)`);
        } else {
          summaryParts.push('In-house $/hr: —');
        }
